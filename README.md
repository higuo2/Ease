...
当前版本 **v1.3**。产品规则见 [`PRD.md`](PRD.md)，视觉与工程约束见 [`AGENT.md`](AGENT.md)。
...
| Tab | 内容 |
|---|---|
| **体重** | 大号当前体重、阶段进度、可配置首页方块（BMI 可点开分段条 + CN/WHO）、近 30 天体重列表 |
| **趋势** | 7 / 30 / 90 / 全部折线、区间统计、达标日估算、睡眠/消耗/经期与体重的交叉对照 |
...
卡路里与宏量营养素、连续打卡、Dark Mode、第三方格式（薄荷 / MyFitnessPal 等）、饮水追踪、围度催打卡通知、桌面/锁屏 Widget、「一键同步 HealthKit」。BMI 不做绿黄红交通灯或医疗结论（首页灰字中国档；Sheet 内安静莫兰迪条可以）。达标日估算用旬区间，不做倒计时。
