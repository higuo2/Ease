...
| Tab | 内容 |
|---|---|
| **体重** | 大号当前体重、阶段进度、可配置首页方块（BMI / 围度 / 体重，可选睡眠、经期、消耗）、近 30 天体重列表 |
...
录入拆成独立 Sheet：体重（可相册识图）、围度。一天可以有多条体重。睡眠、经期、消耗只读 HealthKit，不从 CSV 导入。饮食 / 标签 / 备注 / 餐图只留在 SwiftData 与 CSV 列里，界面不再录入。
