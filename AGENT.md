...
- **ALLOW**: 4-Tab root (Weight / Trend / Calendar / **Settings**); large hero weight number; stage-goal card with linear progress; customizable Morandi home tiles; segmented trend ranges; black chart tooltip (date + last-per-day weight + optional 7-day MA); month calendar with dual-line day cells; weight-history sheet; BMI detail sheet (gray band capsule, quiet Morandi range bar, CN/WHO picker); quiet sleep/period/energy detail tints inside their sheets only; meal photo full-screen preview (system cover, no Hero morph); on-device meal cutout in the meal carousel (quiet subject shadow on the food layer only).
