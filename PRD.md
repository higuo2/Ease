...
采用单页面无 Tab 栏架构。全 App 界面：Dashboard、日更录入 Sheet、围度 Sheet、设置 Sheet、睡眠详情 Sheet、经期详情 Sheet；另加首次启动的 Onboarding。
...
5. **主卡**：所选日**最新一条** `WeightLog` 的体重（大数字）。该条有体脂则在次要行显示。BMI 纯数字，例如 `BMI 21.4`，不做评估语。所选日没有任何 `WeightLog` 时，回退为全局最新一条体重；仍无则显示不可用态，不算进度。
   v1.2：主卡次行之下最多一行 gray 小号字。**任意指标已启用**即显示；点开独立围度 Sheet（不是日更录入）。所选日有已启用指标的 `MetricLog` 时列出最新读数（如 `Waist 68.0 cm`）；没有当日值则只显示入口文案 `Measurements` / `围度`。关掉的指标即使当天已有 log 也不得出现在读数里。全部禁用 → 整行隐藏。不新增马卡龙指标卡。
...
7. **FAB**：底部圆形 `+`，避开 Home Indicator。点击打开**日更**录入 Sheet，日期为 **所选日**（不是永远今天）。不打开围度 Sheet。
...
### 2.2a 围度 Sheet (Metric Sheet)
独立半屏 Modal，与日更录入分开。入口：主卡 gray 行；设置里某指标的 History。内部：日期（默认所选日，不可未来）→ 已启用指标数字行 → Save → 下方该指标历史图与列表。
...
* **录入**：独立围度 Sheet（§2.2a），不塞进日更 Log Sheet。保存时对填了的指标 **insert `MetricLog`**，空行不写。先校验全部再一次写入。不把指标和体重绑在同一条 `WeightLog`。
* **首页**：不新增马卡龙卡。主卡次行之下最多一行 gray 小号数字：**任意 `isEnabled == true` 即出现入口**；有当日 log 的已启用指标列出最新值（如 `Waist 68.0 cm`）；无当日值则只显示入口文案。关掉的指标即使当天已有 `MetricLog` 也**不得**出现在读数里。全部禁用 → 整行隐藏。
...
* **删除 / 禁用**：删一条 `MetricLog` 只删该次。关掉内置或自定义指标只把 `isEnabled = false`，**历史 `MetricLog` 全部保留**（设置 History / 围度 Sheet 仍可看历史）；围度 Sheet 录入区不再出现该字段；首页主卡次行按上条规则立刻不渲染它的读数。清除全部数据时一并删除定义与 log。
