...
* **Non-Goals (绝对禁止)**：
    ...
    * 禁止 BMI 绿黄红交通灯图或彩色诊断 chip。首页 BMI 格用**灰色**中国成人档名。详情 Sheet 可用低饱和莫兰迪分段条 + CN/WHO 对照，档名保持灰色胶囊；禁止写成医疗诊断。
    * 禁止任何社交、社区分享或第三方账号登录。
    * 禁止应用内语言切换。
...
* **本期范围 (v1.3)**：趋势页交叉对照（已有 HealthKit 睡眠 / 消耗 / 经期 × 体重）。规格见 §8.5。
* **仍不做**：桌面 Widget（含锁屏/主屏一键体重或从 Widget 拍照）、体重+体脂双轴图、Dark Mode、第三方格式导入（MyFitnessPal 等）、为扩展指标单独做催打卡通知、卡路里合计、饮水追踪、「一键同步 HealthKit」。**不再提供饮食/标签/备注/餐图录入**（schema 与 CSV 列保留）。不做苹果健康 XML/CSV 导入、不读步数/静息心率/环境噪音、不接日历推断会议原因。
...
5. **交叉对照卡**（§8.5）：高级估算下方。数据不足则整卡隐藏。

交互：预览不改数据；点已有日可编辑该日体重。删体重不得删当日 `DailyRecord` 遗留字段。

### 2.3 日历 Tab（Calendar）
1. 系统 Large Title（`tab.calendar`）。`< 月份 >` 选择器钉在标题下方（奶油底），不滚进状态栏。
2. 7 列月历。星期头：`.secondary` 半粗 caption。每格：日号（等宽）+ 当日体重（`.caption2.monospacedDigit()`）+ 涨跌。未来日用 `.tertiary`，仍可认、不可点。今天 / 选中日用浅珊瑚填（约 15% 透明）+ 1.5pt 细环，日号用主色或珊瑚色，**禁止**白字压实心红圆。
3. 点未选中日期只选中该日；再点同一天打开日明细 Sheet（早晚体重、日间波动 = 同日晚−早）。无餐图、无饮食芯片、无标签、无备注。
4. **月份 Overview 卡**（当前浏览月 / 选中日所在周），禁止三列网格把第四项挤成孤儿行：
   * 上行：月均（左）+ 净变化（右，涨跌用现有 teal / coral）。
   * 发丝分割。
   * 下行四等宽列：周均、打卡天数、减重天数、增重天数。
   周均 = 选中日所在自然周内有记录日的平均；月均 = 当前浏览月内有记录日的平均。**不再统计清淡天数，不再展示「日均变化」横栏。**

### 2.4 设置 Tab（Settings）
第四 Tab（非 Sheet）。无 Done/Close；档案与开关即时保存。顺序：个人 → 提醒 → 首页模块 → 扩展指标 → 数据（CSV）→ 清除全部数据。改初始/目标后阶段进度立刻重算。无饮食提醒、无标签管理页。睡眠 / 经期 / 消耗详情从首页方块打开，**不在设置里再做入口**。

安静系统控制台：**不要**给整页 List 套珊瑚 `.tint`（珊瑚只留给体重进度 / 减重方向）。Toggle 用系统绿；Export / Import 用主色文字；性别、生日、时刻用 secondary；仅「清除全部数据」为 destructive 红。

* **个人**：身高、起始/目标体重、生日（紧凑 DatePicker；未设置显示 Not set，trailing X 清除）、生理性别、**睡眠目标时长**（默认 8.0 h，精度 0.5 h，范围 4–12 h）。
* **提醒**：总开关。关闭时体重提醒时刻行以 0.2s 收起、变淡、不可点。打开后恢复。默认 08:00。
* **扩展指标**：`waist` / `hip` / `chest` / `thigh` / `underbust` 始终在顶层；其余内置 + 自定义收进「更多维度」；「添加指标」在更多组最底部。区头 secondary caption 显示已启用条数。History：行尾 chevron 或左滑；点 History **不得**拨动开关。
* **CSV**：Export / Import 各占一行，不要在 Import 文字上叠 `(i)`。页脚说明一次一个文件：`ease-export.csv`（体重/饮食列）或 `ease-metrics.csv`（围度），表头须完全一致。
...
* 若引擎产出睡眠相关对照，底部可附 **1** 条交叉事实（与趋势卡同一套文案）。
...
入口：首页消耗方块。HealthKit Active Energy 只读；展示当日值、近窗柱状图（带轴）、近期列表与平均。若引擎产出消耗相关对照，底部可附 **1** 条交叉事实。**不设热量目标、不据此给饮食建议。**

### 2.11 BMI 详情 (BMI Detail Sheet)
入口：体重 Tab 的 BMI 方块。只读，不打开体重录入。当前 BMI 大数字 + **灰色**档名胶囊；安静莫兰迪分段条（偏瘦 / 正常 / 超重 / 肥胖）带指针。顶部 CN | WHO 分段切换只影响本 Sheet 的条带、档名和理想体重区间，**不改首页格**（首页永远中国成人切点）。身高 / 体重 / 年龄 / 性别做成 2×2 只读格。底部一张理想体重卡。公式、免责声明、未满 18 / 未填生日脚注放进 `info.circle`。未满 18 仍显示数字，档名为「未评价」，不套成人切点、不画指针。禁止绿黄红交通灯和彩色诊断 chip。
...
* **BMI**：`BMI = displayWeight(kg) / height(m)^2`，精度 0.1。首页方块显示数字 + **灰色**中国成人档名。详情 Sheet：莫兰迪分段条 + CN/WHO 对照（默认 CN）、灰色档名胶囊、输入格、所选标准的正常 BMI 体重区间。公式/免责进 info。禁止绿黄红交通灯和彩色诊断 chip。
    * 中国成人（首页与 Sheet 默认）：&lt; 18.5 偏瘦；18.5–23.9 正常；24.0–27.9 超重；≥ 28.0 肥胖。
    * WHO 成人仅 Sheet 对照：正常到 24.9，超重 25.0–29.9，肥胖 ≥ 30。
    * 有生日且未满 18：仍显示数字，档名为「未评价」，不套成人切点。不做儿童百分位。
    * 无生日：按成人分档，info 脚注说明未填生日。
...
* **多语言**：简体中文与英文；`String Catalog` (.xcstrings)；跟随系统；无应用内切换。界面文案禁止用间隔号「·」做并列（易像生成文本）；用逗号、顿号或拆成两句。
...
### 8.1 CSV 再导入
* **入口**：设置 Tab 里 Export 下方 `Import CSV`，普通按钮行（不要 trailing 说明图标叠在标题上）。系统文件选择器，仅本地文件。页脚说明一次选一个 Ease CSV。
...
* **预览后才落库**：解析完弹出确认 Sheet；用户点 Confirm 才写入。取消则零写入。摘要必须含将写入 / 重复 / 无效（及截断）计数。无效与重复可展开，最多列出 **20** 条样本：CSV 行号（表头为第 1 行）+ 原因（未来日期 / 无法解析 / 越界 / 未知 `metricKey` / 重复）。其余条数只用 count，不要把全部 skipped 行载入预览模型。不做表格 diff，不展示整行原始 CSV。
...
* **文案**：阶段卡下方一行 gray。日期做成**旬区间**（1–10 上旬、11–20 中旬、21–月末下旬；英文 `early / mid / late {Month}`），不要精确到日、不要倒计时天数。距今 ≤7 天用 `later this week` / `本周内`。例如 `At this pace, around mid-October.` / `按近况大约 10 月中旬。`
* **禁止（仅对本层）**：写成「你将成功 / 落后计划」；用热量缺口反推；用睡眠/经期/消耗修正斜率；写成「还有 N 天」。

#### 8.3.B 高级估算（仅趋势 Tab）
* 在 8.3.A 同一套均线 + MAD + OLS 得到基础斜率后，用近期窗口内的睡眠（相对睡眠目标）、活动消耗（相对中位数）、经期日占比做**轻度乘数**（大致范围约 0.88…1.08），得到调整斜率，再推剩余天数与日期。
* 仍遵守同一隐藏护栏与 730 天上限。内部仍算精确 `eta` / `daysRemaining`（测试与隐藏条件用），UI **不**把天数或具体日号当主数字。
* **文案**：主行与 8.3.A 同一套旬桶（如「预计 10 月中旬」）；因子读数保留。冷淡事实语气，不写激励或医疗结论。
* **禁止**：与 8.3.A 合成一句；把消耗写成卡路里缺口目标；经期周期预测文案与本估算混排；大号倒计时天数。

### 8.4 自定义提醒时刻
* 设置在通知总开关下方：仅 `Weight reminder` 的 `hourAndMinute` 选择器。默认 08:00。总开关关闭时该行收起并变淡。`dietReminderHour/Minute` 留在 `UserProfile` 上，UI 不展示、调度器不使用。
...
### 8.5 交叉对照（Health Insights）
目标：用已经在读的 HealthKit 序列对照体重，讲关联不讲故事。不是第二个健康 App，不是 XML 导出分析器。

* **数据**：只读现有序列——按日最后一次 `WeightLog`、asleep 时长、Active Energy、经期日。HealthKit Reader 仍不写 SwiftData；洞察是派生结果，不进 CloudKit。
* **入口**：趋势 Tab，高级估算卡下方。睡眠 / 消耗详情 Sheet 最多各附 1 条本指标相关事实。首页方块、日历、第五 Tab **不加**。
* **窗口**：近 **90** 个日历日。体重日变化只计**相邻日历日**都有 last-per-day 体重的配对。
* **候选（模板化文案，禁止现场编原因）**：
    1. 睡眠不足 6h（与通知同一阈值）的次日体重变化 vs 其余有睡眠数据的日子。
    2. 经期日 vs 非经期日的体重变化。
    3. 某星期几的**夜**（归属为次日「昨晚」，故周三夜记在周四 morning）平均 asleep 最短，且低于其余夜。
    4. 前一日 Active Energy 低于近窗中位数时，次日体重变化 vs 其余有消耗数据的日子。
* **护栏**：每组至少 **4** 个样本；星期模式另需窗口内至少 **14** 个有睡眠的夜。体重差绝对值 &lt; **0.1 kg**、睡眠差绝对值 &lt; **0.5 h** → 隐藏该条。同时最多 **3** 条，按效果量 / 阈值 排序。整组不够则**隐藏整卡**，不写「无法分析」。
* **语气**：冷淡事实；脚注「只描述关联，不是因果，也不是医疗建议」。禁止医疗结论、卡路里缺口、打卡火焰、「因为有会议」。星期几来自本地日历，不读 EventKit。
* **权限**：不新增 HealthKit 类型。无权限或序列为空则视作无对照。
